...
impl ProviderRegistry {
    ...
    fn register_builtin_providers(
        ...
        if config.is_enabled("anthropic") && let Some(entry) = config.get("anthropic") {
            let extra_keys =
        ...
        if config.is_enabled("openai") && let Some(entry) = config.get("openai") {
            let extra_keys = resolve_extra_api_keys(config, "openai", openai_primary_key.as_ref());
            if !extra_keys.is_empty() {
                let base_url = entry
                    .base_url
                    .clone()
